# CodeGraphy theme styling contract

CodeGraphy provides stable semantic tokens and data attributes for themes. Class names remain private unless this document names them.
...
Desktop, local browser, and mobile use the same CodeMirror editor token values. Theme the source editor with the stable editor and syntax variables.

## Stable tokens

The CodeGraphy theme provides these catalog-backed color tokens through `assets/catalog/themes.json`:
...
Optional Catppuccin, Dracula, Atom, and Solarized sources live in this public registry. The [author guide](../README.md) defines the package and publication flow. Executable Plugin loading remains separate future work.
...
The snippet manager imports and exports ordinary `.css` files. Its device library stores each snippet as `snippets/<id>.json` containing its name and CSS. Editing and saving through the manager immediately refreshes enabled snippets. External edits are not watched. Enabled snippet IDs and their order are presentation settings, so Global and Workspace selections can differ while sharing the device library.
...
Markdown callouts also expose the common Obsidian selector shape: `.callout[data-callout="<type>"]`, `.callout-title`, `.callout-icon`, and `.callout-content`. Use `--callout-color` for the accent and `--callout-icon` for a CSS image value. See [Markdown callouts](./markdown-callouts.md) for the complete callout contract.
